...
import com.imnjh.imagepicker.widget.subsamplingview.decoder.CompatDecoderFactory;
import com.imnjh.imagepicker.widget.subsamplingview.decoder.DecoderFactory;
import com.imnjh.imagepicker.widget.subsamplingview.decoder.ImageDecoder;
import com.imnjh.imagepicker.widget.subsamplingview.decoder.ImageRegionDecoder;
import com.imnjh.imagepicker.widget.subsamplingview.decoder.SkiaImageDecoder;
import com.imnjh.imagepicker.widget.subsamplingview.decoder.SkiaImageRegionDecoder;


/**
 * Displays an image subsampled as necessary to avoid loading too much image data into memory. After
 * a pinch to zoom in,
 * a set of image tiles subsampled at higher resolution are loaded and displayed over the base
 * layer. During pinch and
 * zoom, tiles off screen or higher/lower resolution than required are discarded from memory.
 * <p>
 * Tiles are no larger than the max supported bitmap size, so with large images tiling may be used
 * even when zoomed out.
 * <p>
 * v prefixes - coordinates, translations and distances measured in screen (view) pixels s prefixes
 * - coordinates, translations and distances measured in source image pixels (scaled)
 */
...
public class SubsamplingScaleImageView extends View {
    ...
    private static final List<Integer> VALID_ORIENTATIONS = Arrays.asList(ORIENTATION_0,
            ORIENTATION_90, ORIENTATION_180, ORIENTATION_270, ORIENTATION_USE_EXIF);

    /**
     * During zoom animation, keep the point of the image that was tapped in the same place, and scale
     * the image around it.
     */
    ...
    private static final List<Integer> VALID_ZOOM_STYLES = Arrays.asList(ZOOM_FOCUS_FIXED,
            ZOOM_FOCUS_CENTER, ZOOM_FOCUS_CENTER_IMMEDIATE);
    ...
    private static final List<Integer> VALID_EASING_STYLES = Arrays.asList(EASE_IN_OUT_QUAD,
            EASE_OUT_QUAD);
    ...
    private static final List<Integer> VALID_PAN_LIMITS = Arrays.asList(PAN_LIMIT_INSIDE,
            PAN_LIMIT_OUTSIDE, PAN_LIMIT_CENTER);
    ...
    /**
     * Scale the image uniformly so that both dimensions of the image will be equal to or larger than
     * the corresponding dimension of the view. The image is then centered in the view.
     */
    public static final int SCALE_TYPE_CENTER_CROP = 2;
    /**
     * Scale the image so that both dimensions of the image will be equal to or less than the maxScale
     * and equal to or larger than minScale. The image is then centered in the view.
     */
    public static final int SCALE_TYPE_CUSTOM = 3;

    private static final List<Integer> VALID_SCALE_TYPES = Arrays.asList(SCALE_TYPE_CENTER_CROP,
            SCALE_TYPE_CENTER_INSIDE, SCALE_TYPE_CUSTOM);
    ...
    public SubsamplingScaleImageView(Context context, AttributeSet attr) {
        ...
        if (attr != null) {
            TypedArray typedAttr =
                    getContext().obtainStyledAttributes(attr, R.styleable.SubsamplingScaleImageView);
            ...
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_zoomEnabled)) {
                setZoomEnabled(typedAttr
                        .getBoolean(R.styleable.SubsamplingScaleImageView_zoomEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_quickScaleEnabled)) {
                setQuickScaleEnabled(typedAttr.getBoolean(
                        R.styleable.SubsamplingScaleImageView_quickScaleEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_tileBackgroundColor)) {
                setTileBackgroundColor(typedAttr.getColor(
                        R.styleable.SubsamplingScaleImageView_tileBackgroundColor, Color.argb(0, 0, 0, 0)));
            }
            typedAttr.recycle();
        }

        quickScaleThreshold =
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, context.getResources()
                        .getDisplayMetrics());
    }
     ...
     * orientation
     * setting, scale and center. This is the best method to use when you want scale and center to be
     * restored
     * after screen orientation change; it avoids any redundant loading of tiles in the wrong
     * orientation.
     *
     * @param imageSource Image source.
     ...
     * <p>
     * You must declare the dimensions of the full size image by calling
     * {@link ImageSource#dimensions(int, int)} on the imageSource object. The preview source will be
     ...
     * @param previewSource Optional source for a preview image to be displayed and allow interaction
     ...
     * Set the image source from a bitmap, resource, asset, file or other URI, providing a preview
     * image to be
     * displayed until the full size image is loaded, starting with a given orientation setting, scale
     * and center.
     * This is the best method to use when you want scale and center to be restored after screen
     * orientation change;
    ...
    public final void setImage(ImageSource imageSource, ImageSource previewSource,
                               ImageViewState state) {
        LogUtils.d(TAG, "setImage" + "uri:" + imageSource.getUri().toString() + "id"
                ...
                if (uri == null && previewSource.getResource() != null) {
                    uri =
                            Uri.parse(ContentResolver.SCHEME_ANDROID_RESOURCE + "://"
                                    + getContext().getPackageName() + "/" + previewSource.getResource());
                }
                BitmapLoadTask task =
        ...
        if (imageSource.getBitmap() != null && imageSource.getSRegion() != null) {
            onImageLoaded(Bitmap.createBitmap(imageSource.getBitmap(), imageSource.getSRegion().left,
                    imageSource.getSRegion().top, imageSource.getSRegion().width(), imageSource.getSRegion()
                            .height()),
                    ORIENTATION_0, false);
            ...
            if (uri == null && imageSource.getResource() != null) {
                uri =
                        Uri.parse(ContentResolver.SCHEME_ANDROID_RESOURCE + "://"
                                + getContext().getPackageName() + "/" + imageSource.getResource());
            }
    ...
    /**
     * On resize, preserve center and scale. Various behaviours are possible, override this method to
     * use another.
     */
    @Override
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_POINTER_2_DOWN:
                ...
                if (touchCount >= 2) {
                    if (zoomEnabled) {
                        // Start pinch to zoom. Calculate distance between touch points and center point of the
                        // pinch.
                        float distance = distance(event.getX(0), event.getX(1), event.getY(0), event.getY(1));
                        scaleStart = scale;
                        vDistStart = distance;
                        vTranslateStart.set(vTranslate.x, vTranslate.y);
                        vCenterStart.set((event.getX(0) + event.getX(1)) / 2,
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (maxTouchCount > 0) {
                    if (touchCount >= 2) {
                        // Calculate new distance between touch points, to scale and pan relative to start
                        // values.
                        ...
                        if (zoomEnabled
                            ...
                            if (scale <= minScale()) {
                                // Minimum scale reached so don't pan. Adjust start settings so any expand will zoom
                                // in.
                                vDistStart = vDistEnd;
                                scaleStart = minScale();
                                ...
                                // Translate to place the source image coordinate that was at the center of the
                                // pinch at the start
                        ...
                        // One finger pan - translate the image. We do this calculation even with pan disabled
                        // so click
                        ...
                        if (dx > 5 || dy > 5 || isPanning) {
                            ...
                            if (!edgeXSwipe && (!atXEdge || yPan || isPanning)) {
                                ...
                                // Haven't panned the image, and we're at the left or right edge. Switch to page
                                // swipe.
                                maxTouchCount = 0;
                                handler.removeMessages(MESSAGE_LONG_CLICK);
    ...
    private void doubleTapZoom(PointF sCenter, PointF vFocus) {
        ...
        if (doubleTapZoomStyle == ZOOM_FOCUS_CENTER_IMMEDIATE) {
            ...
            new AnimationBuilder(targetScale, sCenter).withInterruptible(false)
                    .withDuration(doubleTapZoomDuration).start();
     ...
     * triggers to calculate
     * the scaling and tiling required. Once the view is setup, tiles are displayed as they are
     * loaded.
     */
    @Override
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        LogUtils.d(TAG, "onDraw" + uri + " id" + System.identityHashCode(this));
        createPaints();
        // If image or view dimensions are not known yet, abort.
        if (sWidth == 0 || sHeight == 0 || getWidth() == 0 || getHeight() == 0) {
            return;
        }
        // When using tiles, on first render with no tile map ready, initialise it and kick off async
        // base image loading.
        ...
        if (anim != null) {
            ...
            scale =
                    ease(anim.easing, scaleElapsed, anim.scaleStart, anim.scaleEnd - anim.scaleStart,
                            anim.duration);

            // Apply required animation to the focal point
            float vFocusNowX =
                    ease(anim.easing, scaleElapsed, anim.vFocusStart.x,
            ...
            // For translate anims, showing the image non-centered is never allowed, for scaling anims it
            // is during the animation.
        ...
        if (tileMap != null && isBaseLayerReady()) {
            ...
            // First check for missing tiles - if there are any we need the base layer underneath to avoid
            // gaps
                ...
                if (tileMapEntry.getKey() == sampleSize || hasMissingTiles) {
                    for (Tile tile : tileMapEntry.getValue()) {
                        ...
                        if (tile.visible && debug) {
                            canvas.drawText("ISS " + tile.sampleSize + " RECT " + tile.sRect.top + ","
                                            + tile.sRect.left + "," + tile.sRect.bottom + "," + tile.sRect.right,
            ...
            if (debug) {
                canvas
                        .drawText("Scale: " + String.format(Locale.ENGLISH, "%.2f", scale), 5, 15, debugPaint);
    ...
    private boolean checkReady() {
        boolean ready =
                getWidth() > 0 && getHeight() > 0 && sWidth > 0 && sHeight > 0
    ...
    private boolean checkImageLoaded() {
        ...
        if (!imageLoadedSent && imageLoaded) {
            ...
            onImageLoaded();
            if (onImageEventListener != null) {
                onImageEventListener.onImageLoaded(sWidth, sHeight);
            }
        }
        return imageLoaded;
    }
    ...
    private synchronized void initialiseBaseLayer(Point maxTileDimensions) {
        ...
        // Load double resolution - next level will be split into four tiles and at the center all four
        // are required,
        ...
        if (fullImageSampleSize == 1 && sRegion == null && sWidth() < maxTileDimensions.x
            ...
            // Whole image is required at native resolution, and is smaller than the canvas max bitmap
            // size.
     ...
     * filled with tiles
     * that are at least as high resolution as the screen. Frees up bitmaps that are now off the
     * screen.
     *
    ...
    private void refreshRequiredTiles(boolean load) {
        ...
        // Load tiles of the correct sample size that are on screen. Discard tiles off screen, and those
        // that are higher
    ...
    private boolean tileVisible(Tile tile) {
        float sVisLeft = viewToSourceX(0), sVisRight = viewToSourceX(getWidth()), sVisTop =
                viewToSourceY(0), sVisBottom = viewToSourceY(getHeight());
     ...
     * and the image on screen. Minimum scale
     * is set so one dimension fills the view and the image is centered on the other dimension. Used
     * to calculate what the target of an
     * animation should be.
     *
    ...
    private void fitToBounds(boolean center, ScaleAndTranslate sat) {
        ...
        float xPaddingRatio =
                getPaddingLeft() > 0 || getPaddingRight() > 0 ? getPaddingLeft()
                        / (float) (getPaddingLeft() + getPaddingRight()) : 0.5f;
        float yPaddingRatio =
                getPaddingTop() > 0 || getPaddingBottom() > 0 ? getPaddingTop()
    ...
    /**
     * Adjusts current scale and translate values to keep scale within the allowed range and the image
     * on screen. Minimum scale
    ...
    private void initialiseTileMap(Point maxTileDimensions) {
        ...
        if (BuildConfig.DEBUG) {
            for (Integer sampleSizeKey : tileMap.keySet()) {
                LogUtils.d(TAG, "[sampleSiz]" + sampleSizeKey + " [tiles]"
    ...
    private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
        ...
        public TilesInitTask(SubsamplingScaleImageView view,
                             Context context, DecoderFactory<? extends ImageRegionDecoder> decoderFactory, Uri source) {
            this.viewRef =
                    new WeakReference<SubsamplingScaleImageView>(view);
        ...
        protected int[] doInBackground(Void... params) {
            try {
                ...
                LogUtils.d(TAG, "TilesInitTask" + sourceUri);
                ...
                if (context != null && decoderFactory != null && view != null) {
                    ...
                    LogUtils.d(TAG, "bitmap width" + sWidth + " bitmap height" + sHeight);
                ...
                LogUtils.e(TAG, "Failed to initialise bitmap decoder", e);
                this.exception = e;
            }
            return null;
        }
    ...
    private static class TileLoadTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        public TileLoadTask(SubsamplingScaleImageView view,
                            ImageRegionDecoder decoder, Tile tile) {
            this.viewRef =
                    new WeakReference<SubsamplingScaleImageView>(view);
            ...
            try {
                ...
                if (decoder != null && tile != null && view != null && decoder.isReady() && tile.visible) {
                    synchronized (view.decoderLock) {
                        ...
                        LogUtils.d(TAG, "TileLoadTask load" + tile.fileSRect);
                ...
                LogUtils.e(TAG, "Failed to decode tile", e);
                this.exception = e;
            } catch (OutOfMemoryError e) {
                LogUtils.e(TAG, "Failed to decode tile - OutOfMemoryError", e);
    ...
    private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
        ...
        public BitmapLoadTask(SubsamplingScaleImageView view,
                              Context context, DecoderFactory<? extends ImageDecoder> decoderFactory, Uri source,
                              boolean preview) {
            this.viewRef =
                    new WeakReference<SubsamplingScaleImageView>(view);
            this.contextRef = new WeakReference<Context>(context);
        ...
        protected Integer doInBackground(Void... params) {
            try {
                String sourceUri = source.toString();
                LogUtils.d(TAG, "BitmapLoadTask" + sourceUri);
                Context context = contextRef.get();
                ...
                LogUtils.e(TAG, "Failed to load bitmap", e);
                this.exception = e;
            } catch (OutOfMemoryError e) {
                LogUtils.e(TAG, "Failed to load bitmap - OutOfMemoryError", e);
                this.exception = new RuntimeException(e);
            }
            return null;
        }
    ...
    private synchronized void onPreviewLoaded(Bitmap previewBitmap) {
        ...
        if (pRegion != null) {
            bitmap =
                    Bitmap.createBitmap(previewBitmap, pRegion.left, pRegion.top, pRegion.width(),
                            pRegion.height());
        } else {
                ...
                if (cursor != null) {
                    if (cursor.moveToFirst()) {
                        ...
                        if (VALID_ORIENTATIONS.contains(orientation) && orientation != ORIENTATION_USE_EXIF) {
                            ...
                            LogUtils.w(TAG, "Unsupported orientation: " + orientation);
                        }
                    }
                    cursor.close();
                }
    ...
    private static class Anim {
        ...
        private PointF sCenterEndRequested; // Source center point that was requested, without
        // adjustment
     ...
     * and force an upper limit to the dimensions of the generated tiles. Passing
     * {@link TILE_SIZE_AUTO} will re-enable the default behaviour.
     *
     * @param maxPixels
     */
    ...
    /**
     * In SDK 14 and above, use canvas max bitmap width and height instead of the default 2048, to
     * avoid redundant tiling.
     */
    ...
    private void fileSRect(Rect sRect, Rect target) {
        if (getRequiredRotation() == 0) {
            ...
            target.set(sWidth - sRect.right, sHeight - sRect.bottom, sWidth - sRect.left, sHeight
                    - sRect.top);
        } else {
     ...
     * significant memory.
     * After you have called this method, the view can be re-used by setting a new image. Settings are
     * remembered
     * but state (scale and center) is forgotten. You can restore these yourself if required.
     */
    public void recycle() {
        LogUtils.d(TAG, "recycle" + uri + " id" + System.identityHashCode(this));
        reset(true);
        bitmapPaint = null;
        debugPaint = null;
        tileBgPaint = null;
     ...
     * with the center
     * adjusted for asymmetric padding. Accepts the desired scale as an argument, so this is
     * independent of current
     * translate and scale. The result is fitted to bounds, putting the image point as near to the
     * screen center as permitted.
     */
    ...
    private float minScale() {
        ...
        if (minimumScaleType == SCALE_TYPE_CENTER_CROP) {
            return Math.max((getWidth() - hPadding) / (float) sWidth(), (getHeight() - vPadding)
                    / (float) sHeight());
        } else if (minimumScaleType == SCALE_TYPE_CUSTOM && minScale > 0) {
            return minScale;
        } else {
            return Math.min((getWidth() - hPadding) / (float) sWidth(), (getHeight() - vPadding)
                    / (float) sHeight());
        }
    }
     ...
     * setting the image file or
     * asset, and you cannot use a custom decoder when using layout XML to set an asset name. Your
     * class must have a
     ...
     *
     * @param regionDecoderFactory The {@link DecoderFactory} implementation that produces
     *                             {@link ImageRegionDecoder} instances.
     */
     ...
     * asset, and you cannot use a custom decoder when using layout XML to set an asset name.
     *
     * @param bitmapDecoderFactory The {@link DecoderFactory} implementation that produces
     *                             {@link ImageDecoder} instances.
     */
    ...
    /**
     * Set the pan limiting style. See static fields. Normally {@link #PAN_LIMIT_INSIDE} is best, for
     * image galleries.
     */
    ...
    /**
     * Set the maximum scale allowed. A value of 1 means 1:1 pixels at maximum scale. You may wish to
     * set this according
     * to screen density - on a retina screen, 1:1 may still be too small. Consider using
     * {@link #setMinimumDpi(int)},
    ...
    /**
     * Set the minimum scale allowed. A value of 1 means 1:1 pixels at minimum scale. You may wish to
     * set this according
     ...
     * express the maximum
     * allowed scale in terms of the minimum pixel density. This avoids the problem of 1:1 scale still
     * being
     * too small on a high density screen. A sensible starting point is 160 - the default used by this
     * view.
     *
    ...
    /**
     * By default, image tiles are at least as high resolution as the screen. For a retina screen this
     * may not be
     * necessary, and may increase the likelihood of an OutOfMemoryError. This method sets a DPI at
     * which higher
     * resolution tiles should be loaded. Using a lower number will on average use less memory but
     * result in a lower
     * quality image. 160-240dpi will usually be enough. This should be called before setting the
     * image source,
     * because it affects which tiles get loaded. When using an untiled source image this method has
     * no effect.
     *
    ...
    /**
     * change the scale and translation of the source image with animation
     *
     * @param scale  New scale to set.
     * @param center New source image coordinate to center on the screen, subject to boundaries.
     */
    public final void setScaleAndCenterWithAnim(float scale, PointF center) {
        new AnimationBuilder(scale, center).withInterruptible(false)
                .withDuration(doubleTapZoomDuration).start();
        invalidate();
    }
    ...
    /**
     * Get source width, ignoring orientation. If {@link #getOrientation()} returns 90 or 270, you can
     * use {@link #getSHeight()} for the apparent width.
     */
    ...
    /**
     * Returns the actual orientation of the image relative to the source file. This will be based on
     * the source file's
     ...
     * double tap is interpreted
     * as a zoom out gesture - if the scale is greater than 90% of this value, a double tap zooms out.
     * Avoid using values
     ...
     * express the scale the
     * image will zoom in to when double tapped in terms of the image pixel density. Values lower than
     * the max scale will
     ...
     * available
     * in SDK 11+. In older versions this has no effect. Parallel loading may use more memory and
     * there
     ...
     * the image in the center of the screen. If doing this would move the image beyond the edges of
     * the screen, the
     * image is instead animated to move the center point as near to the center of the screen as is
     * allowed - it's
     * guaranteed to be on screen.
     *
     * @param sCenter Target center point
     * @return {@link AnimationBuilder} instance. Call
     * {@linkSubsamplingScaleImageView.AnimationBuilder#start()} to start the anim.
     */
    ...
    public final class AnimationBuilder {
        ...
        /**
         * Set the easing style. See static fields. {@link #EASE_IN_OUT_QUAD} is recommended, and the
         * default.
         *
        ...
        /**
         * Only for internal use. When set to true, the animation proceeds towards the actual end point
         * - the nearest
         * point to the center allowed by pan limits. When false, animation is in the direction of the
         * requested end
         * point and is stopped when the limit for each axis is reached. The latter behaviour is used
         * for flings but
         * nothing else.
         */
        ...
        public void start() {
            ...
            PointF targetSCenter =
                    panLimited ? limitedSCenter(this.targetSCenter.x, this.targetSCenter.y, targetScale,
    ...
    public interface OnAnimationEventListener {
        ...
        /**
         * The animation has been aborted before reaching its endpoint because a new animation has been
         * started.
         */
    ...
    /**
     * Default implementation of {@link OnAnimationEventListener} for extension. This does nothing in
     * any method.
     */
